...
export async function proofCaptureSequence() {
  const timeline: { step: string; duration: number; delay: number }[] = [];
 ...
 */
export async function viewTransitionSequence(fromView: string, toView: string) {
  const timeline: { step: string; duration: number; delay: number }[] = [];
 ...
 */
export async function heatShiftSequence(fromMode: 'calm' | 'heat', toMode: 'calm' | 'heat') {
  const timeline: { step: string; duration: number; delay: number }[] = [];
 ...
 */
export async function escalationAlertSequence(severity: 'caution' | 'alert') {
  const timeline: { step: string; duration: number; delay: number; repeats?: number }[] = [];
 ...
 */
export function listCascadeSequence(itemCount: number) {
  const staggerMs = motionConfig.transitions.viewChange.stagger * 1000;
  const timeline: { item: number; delay: number; duration: number }[] = [];
