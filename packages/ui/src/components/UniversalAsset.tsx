...
interface UniversalAssetProps {
  // Flexible querying - can use any combination of filters
  ...
  therapeuticFocus?: string; // From governance tags
...
export function UniversalAsset({
  style,
  dimension = '5:4',
  type,
  description,
  ...
  const [asset, setAsset] = useState<StorageAsset | null>(null);
  ...
  if (loading) {
    return (
      <div className={`universal-asset-loading ${className}`}>
  ...
  if (error || !asset) {
    return (
      <div className={`universal-asset-error ${className}`}>
        <div className="bg-gray-100 rounded-lg h-64 w-full flex items-center justify-center">
          <span className="text-gray-500">Asset not available</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <picture className={`universal-asset ${className}`}>
      {/* AVIF first (best compression) */}
      {asset.type === 'avif' && (
        <source srcSet={asset.url} type="image/avif" />
      )}
...
export function useGovernanceAssets(therapeuticContext: string, limit = 10) {
  const [assets, setAssets] = useState<StorageAsset[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);
