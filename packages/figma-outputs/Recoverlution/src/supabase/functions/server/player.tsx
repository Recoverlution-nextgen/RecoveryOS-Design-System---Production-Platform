/**
 * Universal Player API - Queue Management
 * 
 * Endpoints:
...
player.post('/generate', async (c) => {
  try {
    ...
    // NOW INCLUDES: Clinical NaviCues + Magic Wand NaviCues
    const { data: navicues, error: navicuesError } = await supabase
      .from('navicue_library')
      .select('*')
      .in('track', ['clinical', 'guru', 'magic_wand'])
      .eq('kbe_layer', weakestKBE)
...
player.post('/engagement', async (c) => {
  try {
    // Verify authorization
    const authHeader = c.req.header('Authorization');
    if (!authHeader) {
