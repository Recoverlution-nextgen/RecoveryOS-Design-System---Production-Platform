/**
 * WELLBEING - infiniteK Design System V5
 * 
 * Philosophy: "Your body is part of the recovery"
 * 
 * Premium video library with immersive player overlay
 * - Single hero card: Video library with topic filters
 * - Click video → Launches immersive player (like Journey/NaviCue)
 * - Clean, consistent with platform patterns
 */
...
// Asset for cards (4:5 ratio) - Updated WELLBEING TILE
const WELLBEING_ASSET = "https://owtwhnvntwavuuvkrmvh.supabase.co/storage/v1/object/public/dashboard-assets/Platform/Library/4:5/WebP/WELLBEING_TILE%20-2-2-2.webp";

// The Six Pillars - Using soft, safe colors from colorSystem
const SIX_PILLARS_CONFIG = {
  emotional: {
    clinical: "Emotional Regulation",
    patient: "Feeling It Through",
    icon: Heart,
    color: SIX_PILLARS.emotionalRegulation.primary // #7C67FF - Soft purple
  },
  stress: {
    clinical: "Stress Resilience",
    patient: "Staying Steady",
    icon: Shield,
    color: SIX_PILLARS.stressResilience.primary // #C49DC4 - Soft mauve
  },
  social: {
    clinical: "Social Connectivity",
    patient: "Showing Up",
    icon: Users,
    color: SIX_PILLARS.socialConnectivity.primary // #9D8FFF - Light lavender
  },
  cognitive: {
    clinical: "Cognitive Reframing",
    patient: "Thinking Differently",
    icon: Brain,
    color: SIX_PILLARS.cognitiveReframing.primary // #3E2BB8 - Deep purple
  },
  identity: {
    clinical: "Identity Integration",
    patient: "Becoming Whole",
    icon: Target,
    color: SIX_PILLARS.identityIntegration.primary // #5739FB - Brand purple
  },
  decision: {
    clinical: "Decision Mastery",
    patient: "Choosing Wisely",
    icon: Sparkles,
    color: SIX_PILLARS.decisionMastery.primary // #A8C4E1 - Soft blue
  }
};

type PillarKey = keyof typeof SIX_PILLARS_CONFIG;
...
interface WellbeingPageV5Props {
  patientId?: string | null;
}

export function WellbeingPageV5({ patientId }: WellbeingPageV5Props) {
  ...
  const [videosToShow, setVideosToShow] = useState(9); // Show 9 initially (3x3 grid)
  ...
  const loadFavorites = () => {
    ...
    const mockFavorites = ["yoga-2", "meditation-1", "breathwork-1"];
    setFavoriteVideos(mockFavorites);
    setLoading(false);
  };
  ...
  const handleToggleFavorite = (videoId: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent video from opening
    if (favoriteVideos.includes(videoId)) {
      ...
      console.log(`❤️ Added to favorites: ${videoId}`);
    }
  };

  const handleLoadMore = () => {
    setVideosToShow(prev => prev + 9); // Load 9 more at a time
  };

  const handleTopicChange = (topic: TopicKey | "all") => {
    setSelectedTopic(topic);
    setVideosToShow(9); // Reset to initial amount when changing filters
  };
    ...
    <>
      <div className="flex-1 flex flex-col bg-white overflow-hidden" style={{ height: '100vh' }}>
        ...
        <div className="page-content">
          <div className="content-container">
            ...
            {/* Hero Card: Premium Video Library */}
            <div className="card-hero" style={{ minHeight: '680px' }}>
              ...
              <div style={{
                ...
                <img
                  src={WELLBEING_ASSET}
                  alt="Wellbeing practice library background featuring calming abstract shapes"
                  loading="eager"
                  style={{ 
                    width: '100%', 
                    height: '100%', 
              ...
              <div className="card-hero-content" style={{ 
                ...
                <div style={{ width: '100%' }}>
                  ...
                  <p className="text-body-hero text-white" style={{ maxWidth: '680px', marginBottom: 'var(--spacing-5)' }}>
                  ...
                  <div style={{
                    ...
                    {Object.entries(TOPICS).map(([key, topic]) => {
                      ...
                      return (
                        <button
                          key={key}
                          onClick={() => handleTopicChange(key as TopicKey)}
                          style={{
                            padding: '6px 14px',
                ...
                <div style={{
                  ...
                  {filteredVideos.slice(0, videosToShow).map((video) => {
                    const topic = TOPICS[video.topic];
                    const isFavorite = favoriteVideos.includes(video.id);
                    const primaryPillar = SIX_PILLARS_CONFIG[video.pillarTags[0]];
                    const PillarIcon = primaryPillar.icon;
                    
                    // Extract JW Player media ID from preview URL
                    // Format: https://cdn.jwplayer.com/previews/1J3frLWf-ArinPu6B
                    const mediaId = video.jwPlayerUrl.split('/previews/')[1]?.split('-')[0];
                    const thumbnailUrl = mediaId 
                      ? `https://cdn.jwplayer.com/v2/media/${mediaId}/poster.jpg?width=720`
                      : '';
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          <div style={{
                            ...
                            right: 0,
                            bottom: 0,
                            background: 'linear-gradient(135deg, rgba(62, 43, 184, 0.15), rgba(87, 57, 251, 0.1))',
                            pointerEvents: 'none'
                          }} />

                          {/* Elegant Film Overlay */}
                          <div style={{
                            position: 'absolute',
                            top: 0,
                            left: 0,
                          ...
                          {isFavorite && (
                            <div style={{
                              ...
                              <Heart className="w-3 h-3" style={{ color: '#FFFFFF' }} fill="#FFFFFF" />
                              <span style={{
                                fontFamily: 'var(--font-sans)',
                                fontSize: '0.6875rem',
                                color: '#FFFFFF',
                        ...
                        <div style={{ padding: 'var(--spacing-4)' }}>
                          ...
                          <div style={{
                            ...
                            <div style={{
                              ...
                              <PillarIcon className="w-3 h-3" style={{ color: '#FFFFFF' }} />
                              <span style={{
                                fontFamily: 'var(--font-sans)',
                                fontSize: '0.75rem',
                                color: '#FFFFFF',
                ...
                {/* Load More Button */}
                {filteredVideos.length > videosToShow && (
                  <button
                  ...
                  >
                    ...
                    <span style={{
                      ...
                      {filteredVideos.length - videosToShow} more
                    </span>
                  </button>
                )}
      ...
      {currentVideo && (
        <div style={{
          ...
          {/* Video Player Container - Scrollable */}
          <div style={{
            flex: 1,
            display: 'flex',
            flexDirection: 'column',
            ...
            <div style={{
              ...
              {currentVideo.pillarTags[0] && (
                <p style={{
                  ...
                  {SIX_PILLARS_CONFIG[currentVideo.pillarTags[0]].clinical}
                </p>
              )}
              ...
              <div style={{
                ...
                <button
                  onClick={(e) => handleToggleFavorite(currentVideo.id, e)}
                  style={{
                    background: favoriteVideos.includes(currentVideo.id) 
                      ? 'rgba(255, 69, 0, 0.2)' 
                      : 'rgba(255, 255, 255, 0.1)',
                    border: favoriteVideos.includes(currentVideo.id)
                ...
                >
                  <Heart 
                    className="w-4 h-4" 
                    style={{ 
                      color: favoriteVideos.includes(currentVideo.id) ? '#FF4500' : 'rgba(255, 255, 255, 0.7)'
                    }} 
                    fill={favoriteVideos.includes(currentVideo.id) ? '#FF4500' : 'none'}
                  />
                  <span style={{
                    fontFamily: 'var(--font-sans)',
                    fontSize: '0.875rem',
                    color: favoriteVideos.includes(currentVideo.id) ? '#FF4500' : 'rgba(255, 255, 255, 0.9)',
                    fontWeight: 600
                  }}>
                    {favoriteVideos.includes(currentVideo.id) ? 'Favorited' : 'Add to Toolkit'}
                  </span>
                </button>
              </div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {currentVideo.pillarTags.map(pillarKey => {
                    const pillar = SIX_PILLARS_CONFIG[pillarKey];
