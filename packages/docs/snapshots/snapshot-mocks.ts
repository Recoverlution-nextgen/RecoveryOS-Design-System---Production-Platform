export interface PortalRoom {
  ...
  name: string;
  description: string;
  type: 'individual' | 'professional' | 'organisation';
}
...
export function mockPortalRooms(lens: string): PortalRoom[] {
  const baseRooms = [
    { id: '1', name: 'Welcome', description: 'Getting started', type: 'individual' as const },
    { id: '2', name: 'Dashboard', description: 'Overview and metrics', type: 'professional' as const },
    { id: '3', name: 'Team Hub', description: 'Collaboration space', type: 'organisation' as const },
  ];

  return baseRooms.filter(room => room.type === lens || lens === 'all');
}

export function mockRails(state: string): { primary: RailItem[]; context: RailItem[]; artifact: RailItem[] } {
...
export function mockToasts(type: string): ToastData[] {
  const toasts: Record<string, ToastData[]> = {
    neutral: [
...
export function mockReceipt(state: string): ReceiptData {
